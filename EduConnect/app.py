...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://admin:edu_connect@database-1.cv0s0a2028ju.us-east-1.rds.amazonaws.com/login_signup_db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)  # Initialize SQLAlchemy


# # Replace these placeholders with your actual values
# app.config['MYSQL_HOST'] = 'database-2.ct2wukyuol4a.ap-south-1.rds.amazonaws.com'
# app.config['MYSQL_USER'] = 'admin'  # Your master username
# app.config['MYSQL_PASSWORD'] = 'project-root'  # Your master user's password
# app.config['MYSQL_DB'] = 'mysql'  # Your database name

# Define Models (Below SQLAlchemy Initialization)
class User(db.Model):
...
def login():
    ...
    if request.method == 'POST':
        ...
        account = User.query.filter_by(email=username).first()
        if account:
            if check_password_hash(account.password, password):
                ...
                session['name'] = account.email.split('@')[0]
                msg = 'Logged in successfully!'
                
                return redirect(url_for("home_login", name=session['name']))
            else:
